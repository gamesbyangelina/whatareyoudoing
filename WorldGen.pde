   ...
   while(tiles_to_visit.size() > 0){
     ...
     for(int i=-1; i<2; i++){
       for(int j=-1; j<2; j++){
         ...
         if(map[rx+i][ry+j] == TileType.RIVER || map[rx+i][ry+j] == TileType.STONE){continue;}
         boolean already_visiting = false;
         //If we've already arranged to visit the tile, don't visit it
         for(Pair tv : tiles_to_visit){if(tv.x == rx+i && tv.y == ry+j) already_visiting = true;}
         if(already_visiting){continue;}
         //Otherwise, schedule a visit
         else{tiles_to_visit.add(new Pair(rx+i, ry+j, map[rx+i][ry+j]));}
       }
     }
